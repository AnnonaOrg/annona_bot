...
func DoAdd(item *model.TeleBotInfo) (retText string, err error) {
	return doAddAPI(item)
}

func doAddAPI(req *model.TeleBotInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/telebot/item/add"
