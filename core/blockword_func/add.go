...
func DoAdd(item *model.BlockworldInfo) (retText string, err error) {
	return doAddAPI(item)
}

func doAddAPI(req *model.BlockworldInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/blockword/item/add"
