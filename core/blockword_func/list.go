...
func DoList(item *model.BlockworldInfo) (retText string, err error) {
	return doListAPI(item)
}

func doListAPI(req *model.BlockworldInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/blockword/list"
