...
func DoDel(item *model.BlockformchatidInfo) (retText string, err error) {
	return doDelAPI(item)
}

func doDelAPI(req *model.BlockformchatidInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/blockformchatid/item/del"
