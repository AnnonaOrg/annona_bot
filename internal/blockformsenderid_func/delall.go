...
func DoDelall(item *model.BlockformsenderidInfo) (retText string, err error) {
	return doDelallAPI(item)
}

func doDelallAPI(req *model.BlockformsenderidInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/blockformsenderid/item/delall"
