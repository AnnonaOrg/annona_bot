...
func DoAdd(item *model.KeyworldInfo) (retText string, err error) {
	return doAddAPI(item)
}

func doAddAPI(req *model.KeyworldInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/keyword/item/add"
