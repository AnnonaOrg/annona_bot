...
func DoDelall(item *model.KeyworldInfo) (retText string, err error) {
	return doDelallAPI(item)
}

func doDelallAPI(req *model.KeyworldInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/keyword/item/delall"
