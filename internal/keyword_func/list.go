...
func DoList(item *model.KeyworldInfo) (retText string, err error) {
	return doListAPI(item)
}

func doListAPI(req *model.KeyworldInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/keyword/list"
