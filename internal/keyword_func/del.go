...
func DoDel(item *model.KeyworldInfo) (retText string, err error) {
	return doDelAPI(item)
}

func doDelAPI(req *model.KeyworldInfo) (retText string, err error) {
	apiDomain := osenv.GetCoreApiUrl()
	apiToken := osenv.GetCoreApiToken()
	apiPath := "/apis/v1/keyword/item/del"
