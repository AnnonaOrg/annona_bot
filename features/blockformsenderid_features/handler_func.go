package blockformsenderid_features
...
import (
	...
	model_func "github.com/AnnonaOrg/annona_bot/internal/blockformsenderid_func"
...
func init() {
	features.RegisterFeature("/addblockformsenderid", OnAdd)
	features.RegisterFeature("/delblockformsenderid", OnDel)
	features.RegisterFeature("/delallblockformsenderid", OnDelall)
	features.RegisterFeature("/listblockformsenderid", OnList)
}
...
func OnAdd(c tele.Context) error {
	...
	if len(c.Message().Payload) == 0 {
		return c.Reply(
			"参考指令格式: \n"+
				fmt.Sprintf("```\n%s\n```", "/addblockformsenderid 关键词"),
			tele.ModeMarkdownV2,
		)
	}

	var item model.BlockformsenderidInfo
